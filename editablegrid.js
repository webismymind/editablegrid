...
function Column(config) {
	...
	var props = {
		...
		editable: true,
		renderable: true,
		datatype: "string",  //string, integer, double, email, html, boolean
		unit: null,
		...
		if (metadata && metadata.length >= 1) {
			...
			for (var i = 0; i < columnDeclarations.length; i++) {
				...
				columns.push(new Column({
					...
					vertical_alignment: col.getAttribute("verticalAlignment"),
	...
	if (jsonData.metadata) {
		...
		for (var c = 0; c < jsonData.metadata.length; c++) {
			...
			this.columns.push(new Column({
				...
				vertical_alignment: columndata.verticalAlignment,
